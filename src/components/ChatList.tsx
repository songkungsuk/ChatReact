...
export const ChatList = () => {
    ...
    useEffect(() => {
        ...
        const messageList = async () => {
            //ModelAttribute어노테이션으로 속성값이름만 맞으면 getMapping으로 Key Value를 보냇을때 잘 응답한다.
            const res = await axiosAuth.get(`/chat-list?cmiSenderUiNum=${loginUser.uiNum}&cmiReceiveUiNum=${selectedUser.uiNum}`);
            //응답한 데이터는 페이지 헬퍼로 구성된 객체이기때문에 .list로 한번더 가져온다.
            const resMsg = res.data.list;
            //그렇게 받아온 메세지를 메세지 리스트에 저장
            setMsg(resMsg.reverse());
